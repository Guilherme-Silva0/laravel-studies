    ...
    <nav class="red">
        <div class="nav-wrapper container ">
            ...
            <ul class="right ">
                ...
                <li><a href="#" class="dropdown-trigger" data-target='dropdown2'>Olá {{ auth()->user()->first_name }}!
...
@push('graficos')
    <script>
        ...
        var myChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: Object.keys({{ $userYear }}).map(key => {{ $userYear }}[key]),
                datasets: [{
                    label: [{!! $userLabel !!}],
                    data: Object.keys({{ $userTotal }}).map(key => {{ $userTotal }}[key]),
                    backgroundColor: [
                        'rgba(255, 99, 132, 1)',
                        'rgba(54, 162, 235, 1)',
                        'rgba(255, 206, 86, 1)',
